...
{
    RIO_ASSERT(index < getResParameterListNum());
...
{
    RIO_ASSERT(index < getResParameterObjNum());
    ...
    : ResCommon<ResParameterArchiveData>(p_data)
{
    ...
    RIO_ASSERT(uintptr_t(p_data) % 4 == 0);
