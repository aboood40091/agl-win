...
void ShaderHolder::initialize(SharcArchiveRes* p_arc)
{
    RIO_ASSERT(p_arc != nullptr);
    ...
    for (s32 i = 0; i < cShader_Num; i++)
    {
        ...
        if (program_index < 0)
        {
            RIO_LOG("Cannot find a shader program (name = %s) in the shader archieve (ArchiveType = %d).\n", name, s32(archive));
            RIO_ASSERT(false);
        }

        ShaderProgram* p = const_cast<ShaderProgram*>(program_archive.searchShaderProgram(program_index));
        RIO_ASSERT(p != nullptr);
        mProgram.pushBack(p);
        ...
        case cShader_NV12Decode:
            ...
            p->createUniform(utl::ImageFilter2D::cUniform_Num);
            {
            ...
            p->createSamplerLocation(utl::ImageFilter2D::cSampler_Num);
            {
        ...
        case cShader_XluSnap:
            p->createUniform(utl::ImageFilter2D::cXluSnapUniform_Num);
            {
                p->setUniformName(utl::ImageFilter2D::cUniform_ProjViewWorld,               "uProjViewWorld[0]");
                p->setUniformName(utl::ImageFilter2D::cUniform_TexSize,                     "cTexSize");
            ...
            p->createSamplerLocation(utl::ImageFilter2D::cXluSnapSampler_Num);
            {
                p->setSamplerLocationName(utl::ImageFilter2D::cSampler_Texture,             "uTexture");
        ...
        case cShader_FrameBufferNoFlip:
            ...
            p->createSamplerLocation(utl::ImageFilter2D::cFrameBufferCopySampler_Num);
            {
        ...
        case cShader_DOFVignetting:
            p->createUniform(pfx::DepthOfField::cUniform_Num);
            {
            ...
            p->createSamplerLocation(pfx::DepthOfField::cSampler_Num);
            {
